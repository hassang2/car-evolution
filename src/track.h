...
namespace racingai {
	...
	public:
		Track();
		void update();
		void setup(ofxBox2d* box, vector <shared_ptr<Car> > cars_list, Universe* world, string file_path);
		void draw();
		void center();
		void addEdge();
		void removeEdge();
		...
		void setCars(vector <shared_ptr<Car> > new_cars);
		...
		vector<shared_ptr<Car> > getCars() { return cars_; }
